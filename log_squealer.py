    ...
    def get_banned_ips(self) -> List[str]:
        ...
        try:
            result = subprocess.run(
                ["fail2ban-client", "status", self.jail], 
                capture_output=True, 
                text=True, 
                check=True
            )
...
class AbuseReportGenerator:
    ...
    def _get_fallback_report(self, ip: str, whois_info: Dict[str, Any], server_info: Dict[str, Any]) -> str:
        ...
        return f"""
        Subject: SSH Brute Force Attack Report from {hostname}
...
class SSHAbuseReporter:
    ...
    def get_server_info(self, ip: str) -> Dict[str, Any]:
        ...
        # In a real implementation, this would gather data from logs
        # For now, we'll return basic information
        return {
            "hostname": socket.gethostname(),
            "detection_time": datetime.now().isoformat(),
            "attempt_count": "multiple",  # This would be fetched from actual logs
            "source_ip": ip
        }
